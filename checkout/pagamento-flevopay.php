...
// ===================================================================
// CREDENCIAIS FLEVOPAY
// ===================================================================
define('FLEVOPAY_ACCOUNT_ID', '10243');
define('FLEVOPAY_API_KEY',    'sk_21dd54592a4b8ad4070ac7c17437530e1efe41bd1862a68f93649c8be80b88bc');
define('FLEVOPAY_WEBHOOK_URL', 'https://' . ($_SERVER['HTTP_HOST'] ?? 'seusite.com') . '/checkout/webhook-flevopay.php');
define('FLEVOPAY_UTMIFY_TOKEN', 'XyT0mCfY6s688d00tGgzFLM0oQ0HuPB9pq7E');
// ===================================================================

function getClientIPFlevoPay() {
...
function getProductTitleFlevoPay(int $valor): string {
    switch ($valor) {
...
function getProductTitleFlevoPayGateway(int $valor): string {
    switch ($valor) {
        case 8500:  return 'Caixa de Som Bluetooth 3.0';
        case 6792:  return 'Frete Express';
        case 4780:  return 'Frete PAC';
        case 3890:  return 'Frete FULL';
        default:    return getProductTitleFlevoPay($valor);
    }
}

function sanitizeFlevoPay(string $input, int $max = 255): ?string {
    $s = mb_substr(preg_replace('/[\x00-\x1F\x7F]/u', '', trim($input)), 0, $max, 'UTF-8');
    return $s === '' ? null : $s;
}

function validateNomeFlevoPay(?string $nome): ?string {
    $nome = sanitizeFlevoPay($nome ?? '', 100);
    if (!$nome) return null;
    return preg_match('/^[\p{L}\s\'\-]+$/u', $nome) ? $nome : null;
}

function validateEmailFlevoPay(?string $email): ?string {
    $email = sanitizeFlevoPay($email ?? '', 255);
    return ($email && filter_var($email, FILTER_VALIDATE_EMAIL)) ? $email : null;
}

function validateCPFFlevoPay(?string $cpf): ?string {
    $cpf = preg_replace('/\D/', '', $cpf ?? '');
    return strlen($cpf) === 11 ? $cpf : null;
}

function gerarNomeFlevoPay(): string {
...
function gerarCPFFlevoPay(): string {
    $d = '';
...
try {
    ...
    $nomeVal  = validateNomeFlevoPay($input['nome']     ?? $_POST['nome']     ?? null);
    $emailVal = validateEmailFlevoPay($input['email']   ?? $_POST['email']   ?? null);
    $cpfVal   = validateCPFFlevoPay($input['cpf']       ?? $_POST['cpf']     ?? null);
    ...
    if ($nomeVal && $cpfVal) {
        ...
        $nome_cliente = gerarNomeFlevoPay();
        $cpf          = gerarCPFFlevoPay();
        $email        = strtolower(str_replace(' ', '.', $nome_cliente)) . '@email.com';
        error_log("[FlevoPay] ⚠️ Dados reais inválidos — usando fallback: $nome_cliente");
    }

    $produtoTitulo        = getProductTitleFlevoPay($valor_centavos);
    $produtoTituloGateway = getProductTitleFlevoPayGateway($valor_centavos);
    $reference            = 'FLP-' . time() . '-' . rand(1000, 9999);

    error_log("[FlevoPay] 💰 Valor: $valor_centavos | Produto: $produtoTitulo | Cliente: $nome_cliente");

    // ─── Criar Transação PIX conforme documentação FlevoPay ───────────
    // POST /api/v1/transaction
    $flevoPayBody = [
        'amount'       => $valor_centavos,
        'description'  => $produtoTituloGateway,
        'reference'    => $reference,
        'source'       => 'api_externa',  // Ignora validação de productHash
        'postback_url' => FLEVOPAY_WEBHOOK_URL,
        'customer' => [
            'name'     => $nome_cliente,
            'email'    => $email,
            'document' => $cpf,
    ...
    $ch = curl_init('https://app.flevopay.com.br/api/v1/transaction');
    curl_setopt_array($ch, [
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_POST           => true,
        CURLOPT_POSTFIELDS     => json_encode($flevoPayBody, JSON_UNESCAPED_UNICODE),
        CURLOPT_HTTPHEADER     => [
    ...
    error_log("[FlevoPay] API [$httpCode]: $apiResp");
    ...
    if ($result['status'] !== 'success' || empty($result['transaction_id'])) {
        throw new Exception("Resposta inválida: " . json_encode($result));
    }

    $transactionId = $result['transaction_id'];
    $externalId    = $result['id'];
    $pixQrCode     = $result['qr_code']        ?? '';
    $pixQrBase64   = $result['qr_code_base64'] ?? '';
    $expiresAt     = $result['expires_at']     ?? '';

    error_log("[FlevoPay] ✅ Transação criada: $transactionId (ref: $externalId)");
    ...
    error_log("[FlevoPay] 💾 Salvo no DB: $transactionId");

    // ─── UTMify waiting_payment (direto) ────────────────────────────
    $utmifyWaiting = [
    ...
    curl_setopt_array($chU, [
        ...
        CURLOPT_HTTPHEADER     => [
            'Content-Type: application/json',
            'x-api-token: ' . FLEVOPAY_UTMIFY_TOKEN,
        ],
        CURLOPT_SSL_VERIFYPEER => true,
        CURLOPT_TIMEOUT        => 10,
    ]);
    ...
    $uErr  = curl_error($chU);
    curl_close($chU);
    error_log("[FlevoPay] UTMify waiting_payment [$uCode]: $uResp");
    // Log dedicado do pendente (diagnóstico): payload enviado + resposta da UTMify
    @file_put_contents(
        $logDir . '/utmify-flevopay-pendente-' . date('Y-m-d') . '.log',
    ...
    header('Content-Type: application/json');
    error_log("[FlevoPay] ❌ Erro: " . $e->getMessage());
    echo json_encode([
        'success' => false,
